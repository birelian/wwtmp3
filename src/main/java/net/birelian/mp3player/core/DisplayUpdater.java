...
public class DisplayUpdater {
	...
	public void updateStaticDisplayInformation(PlayerManager playerManager){
		...
		int lenght = playerManager.getSongManager().getCurrentSong().getLength();
		String artist = playerManager.getSongManager().getCurrentSong().getArtist();
		String title = playerManager.getSongManager().getCurrentSong().getTitle();
		String bitrate = playerManager.getSongManager().getCurrentSong().getBitRate();
		String sampleRate = playerManager.getSongManager().getCurrentSong().getSampleRate();
		String channels = playerManager.getSongManager().getCurrentSong().getChannels();
		
		String totalTime = String.format("%02d:%02d", 
			    TimeUnit.SECONDS.toMinutes(lenght),
			    TimeUnit.SECONDS.toSeconds(lenght) - 
			    TimeUnit.MINUTES.toSeconds(TimeUnit.SECONDS.toMinutes((lenght)))
			);
		MainWindow.getInstance().getTxtTotalTime().setText(totalTime);
		MainWindow.getInstance().getTxtTitle().setNewText(artist + " - " + title);				                    	
		MainWindow.getInstance().getTxtMetadata().setText(bitrate + " - " + sampleRate + " - " + channels);	
	}
	/**
	 * Update Dynamic Display. Information that is updated constantly during playback
	 * 
	 * @param playerManager Player manager
	 */
	public void updateDynamicDisplayInformation(PlayerManager playerManager){
		...
		float done = 0;
		float lenght = playerManager.getSongManager().getCurrentSong().getLength();
		...
		String timeElapsed = String.format("%02d:%02d", 
		    ...
		    TimeUnit.MINUTES.toSeconds(TimeUnit.SECONDS.toMinutes((int)elapsed))
		);
		// Set elapsed time
	...
	public void clearDisplay(){
		...
		MainWindow.getInstance().getTxtMetadata().setText("for user instructions");	
	...
	public void updateCurrentSong(PlayerManager playerManager){
		VisualPlayList.getInstance().highlightPlayingSong(playerManager.getSongManager().getCurrentSong());
	}
}
